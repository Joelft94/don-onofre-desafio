    ...
    <div key={product.id}>
      <div className="relative">
        ...
        <div className="relative mt-4">
          ...
          <p className="mt-1 text-sm text-gray-500">{isOpen? product.description : product.description.substring(1, 5)} </p>
        </div>
        <div className="absolute inset-x-0 top-0 flex h-72 items-end justify-end overflow-hidden rounded-lg p-4">
          ...
          />
          <p className="relative text-lg font-semibold text-white">₲{product.price.toLocaleString()}</p>
        </div>
      </div>
      <div className="mt-6">
        <button
